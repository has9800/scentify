        ...
        <div className="cart-wrapper" ref={cartRef}>
            <div className="cart-container">
                <button type="button" className="cart-heading" onClick={() => setShowCart(false)}>
                    ...
                    <span className="cart-num-items light-brown-text font-bold">({totalQuantities} items)</span>
                </button>
                {cartItems.length < 1 && (
                    <div className="empty-cart flex flex-col items-center justify-center">
                        ...
                        <Link href="/">
                            <button type="button" className="light-brown-bg text-white p-2 w-10/12 my-5 font-bold" onClick={() => setShowCart(false)}>
                                Continue shopping
                            </button>
                        </Link>
                    </div>
                ...
                <div className="product-container">
                    {cartItems.length >= 1 && cartItems.map((item) => (
                        <div key={item._id} className="product">
                            <img src={urlFor(item?.image[0])} alt={item._id} className="cart-product-image" />
